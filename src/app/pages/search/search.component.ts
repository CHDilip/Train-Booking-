...
import { Booking, ISearchTrain, Istation, TrainAppBookingPassengers } from 'src/app/models/station';
...
export class SearchComponent implements OnInit {
  ...
  loadStations() {
    this.stationSrv.getAllStations().subscribe((res: any) => {
  ...
  getAllTrains() {
    this.trainSrv.getTrainsBetweenStations(this.searchObj).subscribe((result: any) => {
  ...
  BookTicket() {
    ...
    this.trainSrv.bookTrain(this.bokingObj).subscribe((result: any) => {
      if (result.result) {
